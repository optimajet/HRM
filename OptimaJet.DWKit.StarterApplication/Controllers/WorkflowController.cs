        ...
        public async Task<ActionResult> GetData(string name, string urlFilter)
        {
            try
            {
                ...
                if (entityId.HasValue && (await WorkflowInit.Runtime.IsProcessExistsAsync(entityId.Value)))
                {
                    ...
                    var states = new List<ClientWorkflowState>();
                    if (DWKitRuntime.Security.CheckPermission("Documents", "SetState"))
                    {
                        states = (await WorkflowInit.Runtime.GetAvailableStateToSetAsync(entityId.Value)).Select(s =>
